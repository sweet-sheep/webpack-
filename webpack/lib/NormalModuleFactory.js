			...
			resolver(result, (err, data) => {
				...
				this.hooks.afterResolve.callAsync(data, (err, result) => {
					...
					if (!createdModule) {
						...
						console.log("module: 开始createdModule");
			...
			asyncLib.parallel(
				...
				(err, results) => {
					...
					asyncLib.parallel(
						...
						(err, results) => {
							if (err) return callback(err);
							// 在这里重新赋值排序  results[0] 就是 pre的loader loaders是inline-loader results[1]正常  results[2]滞后的
							...
							process.nextTick(() => {
								...
								console.log("");
								console.log("module: 创建module前环境处理");
								// 这个地方就是整个
								callback(null, {
									context: context,
									request: loaders
